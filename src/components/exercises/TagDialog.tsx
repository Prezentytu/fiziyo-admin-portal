...
export function TagDialog({ open, onOpenChange, tag, organizationId, onSuccess }: TagDialogProps) {
  const isEditing = !!tag;
    ...
    <Dialog open={open} onOpenChange={() => handleCloseAttempt()}>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>{isEditing ? 'Zmień właściwości tagu' : 'Dodaj nowy tag do biblioteki'}</DialogDescription>
        </DialogHeader>
        <Form {...form}>
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                  ...
                  <div className="space-y-1 leading-none">
                    ...
                    <FormDescription>Główne tagi są wyświetlane jako pierwsze</FormDescription>
                  </div>
                </FormItem>
              )}
            />
