...
function TierBadge({
  ...
  const Icon = tier === 'ELITE' ? Crown : tier === 'EXPERT' ? Star : tier === 'PRO' ? Zap : Trophy;
...
export function CommissionTierCard({ organizationId, className }: CommissionTierCardProps) {
  // Fetch tier info
  ...
  const progressData = useMemo(() => {
    ...
    if (currentTier === 'START' && tierInfo.nextTierThreshold) {
      ...
      progressPercent = Math.min(100, ((tierInfo.activeSubscribers - startMin) / (proMin - startMin)) * 100);
      nextTierName = 'PRO';
    } else if (currentTier === 'PRO' && tierInfo.nextTierThreshold) {
      const proMin = COMMISSION_TIERS.PRO.minSubscribers;
      const expertMin = COMMISSION_TIERS.EXPERT.minSubscribers;
      progressPercent = Math.min(100, ((tierInfo.activeSubscribers - proMin) / (expertMin - proMin)) * 100);
      nextTierName = 'EXPERT';
    } else if (currentTier === 'EXPERT' && tierInfo.nextTierThreshold) {
      const expertMin = COMMISSION_TIERS.EXPERT.minSubscribers;
      const eliteMin = COMMISSION_TIERS.ELITE.minSubscribers;
      progressPercent = Math.min(100, ((tierInfo.activeSubscribers - expertMin) / (eliteMin - expertMin)) * 100);
      nextTierName = 'ELITE';
    ...
    return (
      <Card className={cn('border-border/60', className)}>
        <CardContent className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-sm text-muted-foreground">Nie udało się pobrać danych o prowizji</p>
        </CardContent>
      </Card>
    );
  }
  ...
  if (progressData.isPartner) {
    return (
      <Card className={cn('border-primary/30 bg-gradient-to-br from-surface to-primary/5', className)}>
        <CardHeader className="pb-3">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{progressData.activeSubscribers} aktywnych subskrybentów</span>
          </div>
        </CardContent>
      </Card>
    );
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg font-bold text-foreground">{formatPercent(progressData.commissionRate)}</span>
          </div>
          ...
          <>
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between text-xs">
                <span className="text-muted-foreground">{progressData.currentTier}</span>
                <span className="text-muted-foreground">{progressData.nextTierName}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 p-3 rounded-lg bg-primary/10 border border-primary/20">
              ...
              <div className="text-sm text-foreground">
                🎯 Brakuje Ci <span className="font-bold text-primary">{progressData.progressToNext} pacjentów</span> do
                poziomu{' '}
                ...
                {progressData.nextTierRate && (
                  <span className="text-muted-foreground">
                    {' '}
                    (+
                    {Math.round((progressData.nextTierRate - progressData.commissionRate) * 100)}% prowizji!)
                  </span>
                )}
              </div>
            </div>
        ...
        <div className="grid grid-cols-4 gap-2">
          ...
          <TierBadge
            ...
            isActive={progressData.activeSubscribers >= COMMISSION_TIERS.PRO.minSubscribers}
            isCurrent={progressData.currentTier === 'PRO'}
          />
          <TierBadge
            tier="EXPERT"
            rate={COMMISSION_TIERS.EXPERT.rate}
            isActive={progressData.activeSubscribers >= COMMISSION_TIERS.EXPERT.minSubscribers}
            isCurrent={progressData.currentTier === 'EXPERT'}
          />
          <TierBadge
            tier="ELITE"
            rate={COMMISSION_TIERS.ELITE.rate}
            isActive={progressData.activeSubscribers >= COMMISSION_TIERS.ELITE.minSubscribers}
