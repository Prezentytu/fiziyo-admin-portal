...
import type { GetPatientInviteLinksResponse, CancelPatientInviteLinkResponse } from '@/types/apollo';
...
export function PatientInvitesCard({ organizationId, className }: PatientInvitesCardProps) {
  const [dialogOpen, setDialogOpen] = useState(false);

  // Fetch invites
  const { data, loading, error, refetch } = useQuery<GetPatientInviteLinksResponse>(GET_PATIENT_INVITE_LINKS_QUERY, {
  ...
  const [cancelInvite] = useMutation<CancelPatientInviteLinkResponse>(CANCEL_PATIENT_INVITE_LINK_MUTATION, {
    refetchQueries: [
      {
        ...
        <CardContent>
          {error ? (
            <div className="text-center py-8">
              <p className="text-sm text-destructive">Nie udało się pobrać zaproszeń</p>
            </div>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm font-medium text-foreground mb-1">Brak zaproszeń</p>
              <p className="text-xs text-muted-foreground max-w-sm mx-auto">
                Zaproś pacjentów do wykupienia subskrypcji Premium i zacznij zarabiać prowizje.
              </p>
            </div>
          ) : (
            <div className="space-y-2">
              {sortedInvites.slice(0, 10).map((invite) => (
                <InviteRow key={invite.id} invite={invite} onCancel={() => handleCancel(invite.id)} />
              ))}
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          <p className="font-medium text-foreground truncate">
            {invite.patientName || invite.patientEmail || invite.patientPhone || 'Bez danych'}
          </p>
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            {invite.status === 'pending' && daysUntilExpiry > 0 && <span>Wygasa za {daysUntilExpiry} dni</span>}
            {invite.status === 'used' && invite.usedAt && (
              <span>Użyty {new Date(invite.usedAt).toLocaleDateString('pl-PL')}</span>
            )}
          </div>
        </div>
      </div>
