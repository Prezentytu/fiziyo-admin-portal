...
import { History, ArrowDownRight, ArrowUpRight, User, Calendar, Filter, ChevronDown } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 min-w-0">
        ...
        {transaction.patientUser ? (
          <Avatar className="h-9 w-9">
            <AvatarImage src={transaction.patientUser.image ?? undefined} alt={transaction.patientUser.fullname} />
        ...
        <div className="min-w-0">
          <p className="font-medium text-foreground truncate">
            {transaction.description || transaction.patientUser?.fullname || 'Transakcja'}
          </p>
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <p className={cn('text-sm font-bold tabular-nums', isPositive ? 'text-emerald-500' : 'text-foreground')}>
          {isPositive ? '+' : ''}
          {formatCurrency(
...
export function RevenueHistoryTable({ organizationId, className }: RevenueHistoryTableProps) {
  const [filter, setFilter] = useState<FilterType>('all');
  const [showAll, setShowAll] = useState(false);

  // Fetch revenue history
  const { data, loading, error } = useQuery<GetRevenueHistoryResponse>(GET_REVENUE_HISTORY_QUERY, {
    variables: {
      ...
      <CardContent>
        {error ? (
          <div className="text-center py-8">
            <p className="text-sm text-destructive">Nie udało się pobrać historii transakcji</p>
          </div>
