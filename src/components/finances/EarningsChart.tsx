...
export function EarningsChart({ organizationId, className }: EarningsChartProps) {
  // Fetch 12 months of earnings data
  const { data, loading, error } = useQuery<GetMonthlyEarningsSummaryResponse>(GET_MONTHLY_EARNINGS_SUMMARY_QUERY, {
          ...
          <div className="flex items-center gap-3">
            ...
            {chartData.trend !== 0 && (
              <Badge
                ...
                className={cn(
                  'text-xs border-0',
                  chartData.trend > 0 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'
                )}
              >
                {chartData.trend > 0 ? (
