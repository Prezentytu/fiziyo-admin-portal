 ...
 */
export function StripeReturnHandler({ organizationId }: StripeReturnHandlerProps) {
  ...
  const { refetch, data: _data } = useQuery<GetStripeConnectStatusResponse>(GET_STRIPE_CONNECT_STATUS_QUERY, {
    ...
    const handleReturn = async () => {
      ...
      if (stripeReturn === 'success') {
        ...
        try {
          ...
          if (status?.onboardingComplete) {
            toast.success('Konto Stripe zostało pomyślnie skonfigurowane!', {
              description: 'Teraz możesz otrzymywać wypłaty prowizji od subskrypcji pacjentów.',
              icon: <CheckCircle className="h-5 w-5 text-emerald-500" />,
              duration: 5000,
            });
          } else if (status?.hasConnectedAccount) {
            toast.info('Konto Stripe częściowo skonfigurowane', {
              description: 'Dokończ konfigurację w Stripe, aby móc otrzymywać wypłaty.',
              duration: 5000,
            });
          } else {
        ...
        toast.info('Konfiguracja anulowana', {
          description: 'Możesz wrócić do konfiguracji konta Stripe w dowolnym momencie.',
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
        <div className="flex flex-col items-center gap-4 p-8 rounded-2xl bg-surface border border-border shadow-xl">
          ...
          <div className="text-center">
            <p className="font-medium text-foreground">Weryfikacja statusu konta...</p>
