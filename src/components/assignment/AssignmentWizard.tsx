...
export function AssignmentWizard({
  ...
  const [overrides, setOverrides] = useState<Map<string, ExerciseOverride>>(
    new Map()
  );
  ...
  useEffect(() => {
    if (open) {
      const dynamicSteps = getWizardSteps(mode, !!preselectedSet, !!preselectedPatient);
      setCurrentStep(dynamicSteps[0]?.id || "select-set");
      setCompletedSteps(new Set());
      setSelectedSet(preselectedSet || null);
      setSelectedPatients(preselectedPatient ? [preselectedPatient] : []);
      setOverrides(new Map());
      setStartDate(new Date());
      setEndDate(addDays(new Date(), 30));
      setFrequency(defaultFrequency as Frequency);
    }
  }, [open, preselectedSet, preselectedPatient, mode]);
    ...
    switch (currentStep) {
      ...
      case "customize":
        return {
          ...
          description:
            "Dostosuj parametry ćwiczeń dla wybranych pacjentów (opcjonalnie)",
        };
      case "schedule":
        return {
          title: "Harmonogram",
        ...
        await assignSet({
          variables: {
            ...
            frequency: {
              ...
              timesPerWeek: String(
                Object.values(frequency).filter((v) => v === true).length
              ),
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl w-[95vw] max-h-[90vh] h-[85vh] flex flex-col p-0 gap-0">
        {/* Header */}
