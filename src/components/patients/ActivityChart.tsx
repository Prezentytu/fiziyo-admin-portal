...
export function ActivityChart({
  ...
  if (data.length === 0) {
    return <div className={cn('text-center py-8 text-muted-foreground', className)}>Brak danych do wyświetlenia</div>;
  }
    ...
    return (
      <div className={cn('space-y-3', className)}>
        ...
        <div className="space-y-4">
          {data.map((item, index) => {
            const percentage = item.maxValue && item.maxValue > 0 ? Math.round((item.value / item.maxValue) * 100) : 0;
          ...
          return (
            <div key={index} className="flex-1 flex flex-col items-center gap-2">
              <div className="relative w-full flex items-end justify-center h-24">
                <div
                  className={cn('w-full max-w-[40px] rounded-t-lg transition-all duration-500', colorClasses[color])}
                  style={{ height: `${Math.max(height, 4)}%` }}
                  title={`${item.label}: ${item.value}`}
                />
                {item.value > 0 && (
                  <span className="absolute -top-5 text-xs font-semibold text-foreground">{item.value}</span>
                )}
              </div>
              {showLabels && (
    ...
    <div className={cn('flex gap-2', className)}>
      {dayStatuses.map((day, index) => (
        <div key={index} className="flex flex-col items-center gap-2">
          ...
          <span className="text-[10px] text-muted-foreground">{dayLabels[day.day] || day.day}</span>
        </div>
      ))}
    </div>
  );
