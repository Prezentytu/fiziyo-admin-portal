...
export function InlineEditField({
  ...
  if (status === 'editing' || status === 'saving') {
    ...
    const fontClasses =
      className
        ?.split(' ')
        .filter((c) => c.startsWith('text-') || c.startsWith('font-') || c === 'break-words')
        .join(' ') || '';
    ...
    return (
      <div ref={containerRef} className={cn('relative w-full max-w-full overflow-hidden')} data-testid={testId}>
        {type === 'textarea' || multiline ? (
          ...
          <Select value={String(editValue ?? '')} onValueChange={handleSelectChange} disabled={status === 'saving'}>
            <SelectTrigger className={cn('w-full', status === 'saving' && 'opacity-70', isCompact && 'h-8 text-sm')}>
          ...
          <Input
            ...
            value={editValue ?? ''}
            onChange={(e) =>
              setEditValue(type === 'number' ? (e.target.value === '' ? null : Number(e.target.value)) : e.target.value)
            }
            onBlur={handleBlur}
            onKeyDown={handleKeyDown}
            placeholder={placeholder}
  ...
  // Read mode
  const displayValue = displayFormat ? displayFormat(value) : (value ?? placeholder ?? 'Kliknij aby edytować');
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isGhost
          ...
          : cn(status === 'hover' && 'bg-surface-light'),
        // Status colors
    ...
    >
      ...
      {status === 'error' && <X className="h-4 w-4 text-destructive shrink-0" />}
...
export function InlineEditSelect({
  ...
  return (
    <div className="relative" data-testid={testId}>
      <Select value={optimisticValue} onValueChange={handleChange} disabled={disabled || status === 'saving'}>
        <SelectTrigger
          className={cn(
            'transition-all',
            // Size
            ...
            // Ghost variant
            isGhost &&
              'border-transparent bg-transparent hover:border-border/60 hover:bg-surface-light/50 focus:border-primary focus:bg-surface',
            // Status
            status === 'saving' && 'opacity-70',
