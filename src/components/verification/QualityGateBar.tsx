...
export function QualityGateBar({
  ...
  return (
    <div className={cn('rounded-lg border transition-all', statusConfig.bgClass, className)} data-testid={testId}>
      {/* Main bar */}
      <button
        type="button"
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className={cn('text-sm font-medium', statusConfig.textClass)}>{statusConfig.text}</span>
        </div>
        ...
        <div className="px-3 pb-3 pt-1 space-y-2 border-t border-inherit/50">
          ...
          {criticalMissing.length > 0 && (
            <div className="space-y-1">
              <span className="text-xs font-medium text-red-600 uppercase tracking-wide">Wymagane</span>
              <div className="flex flex-wrap gap-1.5">
                {criticalMissing.map((field) => (
                  <Badge key={field} variant="outline" className="text-xs bg-red-500/10 text-red-600 border-red-500/30">
                    {field}
                  </Badge>
                ))}
              </div>
          ...
          {recommendedMissing.length > 0 && (
            <div className="space-y-1">
              <span className="text-xs font-medium text-amber-600 uppercase tracking-wide">Sugerowane</span>
              <div className="flex flex-wrap gap-1.5">
