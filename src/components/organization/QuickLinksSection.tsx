...
function QuickLinkCard({ href, icon: Icon, label, count, description, color }: QuickLinkCardProps) {
  ...
  return (
    ...
    >
      <div className={cn('flex h-12 w-12 items-center justify-center rounded-xl shrink-0', colors.bg)}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {count !== undefined && <span className={cn('text-lg font-bold', colors.count)}>{count}</span>}
        </div>
...
export function QuickLinksSection({ patientsCount, exercisesCount, setsCount, className }: QuickLinksSectionProps) {
  return (
    <div className={cn('space-y-3', className)}>
      <h3 className="text-sm font-medium text-muted-foreground px-1">Szybki dostęp</h3>
