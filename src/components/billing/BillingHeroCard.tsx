...
export function BillingHeroCard({ organizationId, className }: BillingHeroCardProps) {
  ...
  if (loading) {
    return (
      <div
        className={cn(
          'relative overflow-hidden rounded-2xl bg-gradient-to-br from-surface via-surface to-background border border-border p-6',
          className
        )}
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
          <div className="space-y-3">
            <Skeleton className="h-5 w-48 bg-surface-light" />
            <Skeleton className="h-12 w-40 bg-surface-light" />
            <Skeleton className="h-4 w-32 bg-surface-light" />
          </div>
          <div className="flex flex-wrap gap-3">
            <Skeleton className="h-10 w-36 bg-surface-light rounded-xl" />
            <Skeleton className="h-10 w-36 bg-surface-light rounded-xl" />
          </div>
        </div>
      </div>
    ...
    return (
      ...
      >
        <div className="flex flex-col items-center justify-center text-center py-4">
          ...
          <p className="text-xs text-text-tertiary mt-1">{error?.message || 'Spróbuj ponownie później'}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div
      className={cn(
        'group relative overflow-hidden rounded-2xl bg-gradient-to-br from-surface via-surface to-background border border-border p-6 transition-all duration-300 hover:shadow-xl hover:shadow-primary/10',
        className
      )}
      data-testid="billing-hero-card"
    >
      ...
      <div className="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <h2 className="text-sm font-medium text-muted-foreground">Bieżące Rozliczenie</h2>
              <Badge variant="secondary" className="gap-1.5 bg-surface-light text-foreground border-0">
              ...
              <TooltipTrigger asChild>
                <div className="cursor-help">
                  ...
                  >
                    ...
                    <span className="text-xl sm:text-2xl text-muted-foreground ml-2">{displayData.currency}</span>
                  </p>
                </div>
              </TooltipTrigger>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <div className="flex items-center gap-3 rounded-xl bg-surface-light/50 backdrop-blur-sm border border-border px-4 py-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Aktywnych Pacjentów</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 rounded-xl bg-surface-light/50 backdrop-blur-sm border border-border px-4 py-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Stawka / Licencja</p>
            </div>
          </div>
          ...
          <Button
            variant="outline"
            size="sm"
            className="h-auto py-3 px-4 rounded-xl border-border bg-surface-light/30 text-foreground hover:bg-surface-light hover:text-white hover:border-border-light transition-all"
