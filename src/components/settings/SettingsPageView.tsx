...
import { ProfileForm } from '@/components/settings/ProfileForm.dynamic';
import type { UserProfile } from '@/components/settings/ProfileForm';
...
import type { UserOrganizationsResponse, OrganizationByIdResponse } from '@/types/apollo';

export function SettingsPageView() {
  const { user: clerkUser } = useUser();
  const { user: currentUser, isLoading: userLoading, refetch: refetchUser } = useCurrentUser();
  ...
  const userByClerkId = currentUser;
    ...
    <div className="flex min-h-full -m-4 lg:-m-6 2xl:-m-8 bg-background">
      ...
      >
        ...
        <div className="w-64 border-r border-border/40 bg-transparent flex flex-col shrink-0">
          <div className="p-6">
            <PageHeader title="Ustawienia" titleTestId="settings-page-title" className="mb-6" />
