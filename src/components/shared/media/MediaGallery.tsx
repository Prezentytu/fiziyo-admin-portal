...
export function MediaGallery({
  ...
  const stageBackgroundSrc = selectedItem?.kind === 'video' ? (selectedItem.poster ?? undefined) : selectedItem?.src;
  ...
  const stageWrapperClass =
    layout === 'fill'
      ...
      : cn('relative overflow-hidden', ASPECT_RATIO_CLASS[aspectRatio]);

  return (
    <TooltipProvider>
      <div
        className={cn(
          'relative flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-border/40 bg-card',
          className
        )}
