  ...
  const content = (
    <Card
      className={cn(
        ...
        href && 'cursor-pointer hover:border-border/80 hover:bg-surface-light/50',
        isHero && 'h-full',
        className
      )}
    >
      {/* Subtle gradient background for hero */}
      {isHero && <div className={cn('absolute inset-0 bg-gradient-to-br opacity-60', variantGradients[variant])} />}

      <CardContent className={cn('relative', isHero ? 'p-6 h-full flex flex-col' : 'p-6')}>
        <div className={cn('flex items-start justify-between', isHero && 'mb-4')}>
          <div className="space-y-2">
            <p className={cn('font-medium text-muted-foreground', isHero ? 'text-base' : 'text-sm')}>{title}</p>
            {loading ? (
              <Skeleton className={cn(isHero ? 'h-14 w-28' : 'h-10 w-20')} />
            ) : (
              <p className={cn('font-bold tracking-tight text-foreground', isHero ? 'text-5xl' : 'text-3xl')}>
                {value}
              </p>
            )}
          ...
          <div
            className={cn(
              ...
              variant === 'primary' && 'bg-gradient-to-br from-primary/90 to-primary-dark/90',
              variant === 'secondary' && 'bg-gradient-to-br from-secondary/90 to-teal-600/90',
              variant === 'info' && 'bg-gradient-to-br from-info/90 to-blue-600/90',
              variant === 'warning' && 'bg-gradient-to-br from-warning/90 to-amber-600/90',
