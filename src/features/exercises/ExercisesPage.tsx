...
import { Plus, Dumbbell, LayoutGrid, List, Sparkles } from 'lucide-react';
...
export function ExercisesPage() {
  ...
  const { data, loading, error, refetch } = useQuery(GET_AVAILABLE_EXERCISES_LIST_QUERY, {
  ...
  const [deleteExercise, { loading: deleting }] = useMutation(DELETE_EXERCISE_MUTATION, {
    refetchQueries: [
      {
        query: GET_AVAILABLE_EXERCISES_LIST_QUERY,
        variables: { organizationId },
      },
    ],
  });
      ...
      <div className="flex-1 overflow-y-auto p-4 lg:p-6 scroll-smooth custom-scrollbar">
        <div className="max-w-screen-2xl mx-auto space-y-6">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <PageHeader title="Ćwiczenia" titleTestId="exercise-page-title" className="flex-1" />
          ...
          <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-12">
            ...
            <PageHero
              ...
              description="Utwórz własne"
              icon={<Plus className="h-5 w-5 text-primary-foreground" />}
              onClick={() => setIsDialogOpen(true)}
              disabled={!organizationId}
              testId="exercise-create-btn"
            />
            ...
            <div className="grid grid-cols-3 gap-3 sm:col-span-1 lg:col-span-8">
              ...
              >
                <div className="flex items-center gap-2">
                  <Dumbbell
                    className={cn('h-4 w-4', sourceFilter === 'organization' ? 'text-info' : 'text-muted-foreground')}
                  />
                  <span
                    className={cn(
                      'text-2xl font-bold',
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span
                    className={cn('text-2xl font-bold', sourceFilter === 'fiziyo' ? 'text-violet' : 'text-foreground')}
                  >
                    {fiziyoCount}
                  </span>
                </div>
          ...
          {loading ? (
            ...
            >
              <ListSkeleton variant={viewMode === 'grid' ? 'grid' : 'rows'} count={8} />
            </div>
          ...
          ) : viewMode === 'grid' && filteredExercises.length > 50 ? (
            <VirtualizedGrid
              ...
              renderItem={(exercise) => (
                <ExerciseCard
                  ...
                  onAddToSet={() => {}}
                  onSubmitToGlobal={(item) => setSubmitToGlobalExercise(item)}
                  onSubmitToOrganizationReview={(item) => setSubmitToOrganizationExercise(item)}
                  onReportIssue={(item) => setReportExercise(item)}
