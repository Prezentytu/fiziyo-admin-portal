...
export function ExerciseDetailPage({ params }: ExerciseDetailPageProps) {
  const { id } = use(params);
  ...
  const { data, loading, error, refetch } = useQuery(GET_EXERCISE_BY_ID_QUERY, {
    variables: { id },
  });
  ...
  const [createExercise, { loading: duplicating }] = useMutation(CREATE_EXERCISE_MUTATION, {
    refetchQueries: organizationId ? [{ query: GET_ORGANIZATION_EXERCISES_QUERY, variables: { organizationId } }] : [],
  });
  ...
  const handleDuplicateExercise = async () => {
    ...
    try {
      ...
      const duplicatedExerciseId = (result.data as { createExercise?: { id?: string } } | undefined)?.createExercise
        ?.id;
  ...
  if (error || !exercise) {
    return (
      <ErrorState
        title={error ? 'Nie udało się wczytać ćwiczenia' : 'Nie znaleziono ćwiczenia'}
        description={error?.message}
        onRetry={error ? () => void refetch() : () => router.push('/exercises')}
      />
    );
  }
              ...
              {exercise.videoUrl && (
                <DropdownMenuItem asChild>
                  <a
                    ...
                    data-testid="exercise-detail-open-video-link"
                  >
        ...
        >
          ...
          <div className="relative flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-sm text-primary-foreground/70">Użyj w zestawie</p>
            </div>
            ...
            <div className="relative flex items-center gap-4">
              ...
              <div className="min-w-0 flex-1">
                <h3 className="text-base font-bold text-foreground">
                  {isEditing ? 'Zakończ edycję' : 'Edytuj ćwiczenie'}
                </h3>
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Button
                  ...
                  disabled={
                    uploadingImage || isGeneratingAiImage || !exercise.name || (exercise.images?.length ?? 0) >= 5
                  }
