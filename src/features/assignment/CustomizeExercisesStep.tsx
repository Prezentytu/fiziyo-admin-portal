...
export function CustomizeExercisesStep({
  ...
  const getEffectiveValue = useCallback(
    <T extends string | number | string[] | undefined>(mapping: ExerciseMapping, field: keyof ExerciseOverride): T => {
  ...
  const updateOverride = useCallback(
    (mappingId: string, field: keyof ExerciseOverride, value: number | string | string[] | undefined) => {
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1">
          <Input
            ...
            value={displayValue}
            onChange={(e) =>
              updateOverride(mapping.id, field, e.target.value ? Math.max(min, Number(e.target.value)) : undefined)
            }
            placeholder="—"
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full min-h-0 p-6">
      ...
      <div className="flex flex-col min-h-0">
        <div className="mb-4">
          ...
          <p className="text-sm text-muted-foreground">Kliknij na ćwiczenie, aby je spersonalizować</p>
        </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    >
                      {getEffectiveValue<number | undefined>(mapping, 'sets') && (
                        <span>{getEffectiveValue<number>(mapping, 'sets')} serie</span>
                      )}
        ...
        <div className="mt-3 flex items-center gap-2 flex-wrap">
          <Badge variant="secondary">{mappings.filter((m) => hasOverride(m.id)).length} zmienione</Badge>
          {excludedExercises.size > 0 && <Badge variant="destructive">{excludedExercises.size} wykluczone</Badge>}
          <span className="text-xs text-muted-foreground">z {mappings.length} ćwiczeń</span>
        </div>
      </div>
      ...
      <div className="flex flex-col min-h-0 rounded-xl border border-border bg-surface/50 overflow-hidden">
        {selectedMapping ? (
            ...
            <ScrollArea className="flex-1">
              <div className="p-4 space-y-3">
                ...
                <div className="space-y-2 pt-2">
                  ...
                  <Textarea
                    value={
                      getEffectiveValue<string | undefined>(selectedMapping, 'notes') ??
                      getDefaultValue<string | undefined>(selectedMapping, 'notes') ??
                      ''
                    }
                    onChange={(e) => updateOverride(selectedMapping.id, 'notes', e.target.value || undefined)}
                ...
                <Collapsible>
                  ...
                  <CollapsibleContent className="pt-4 space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        value={
                          getEffectiveValue<string | undefined>(selectedMapping, 'customName') ??
                          getDefaultValue<string | undefined>(selectedMapping, 'customName') ??
                          ''
                        }
                        onChange={(e) => updateOverride(selectedMapping.id, 'customName', e.target.value || undefined)}
                    ...
                    <div className="space-y-2">
                      ...
                      <Textarea
                        value={
                          getEffectiveValue<string | undefined>(selectedMapping, 'customDescription') ??
                          getDefaultValue<string | undefined>(selectedMapping, 'customDescription') ??
                          ''
                        }
                        onChange={(e) =>
                          updateOverride(selectedMapping.id, 'customDescription', e.target.value || undefined)
                        }
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
            ...
            <p className="text-sm font-medium text-muted-foreground">Wybierz ćwiczenie</p>
