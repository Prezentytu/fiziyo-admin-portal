  ...
  return (
    <div className="flex flex-col w-full">
      <div className="flex items-center justify-between bg-surface-light/50 border border-border/40 rounded-xl p-2 group hover:border-border/60 transition-colors">
        ...
        <div className="font-bold text-3xl text-foreground tabular-nums min-w-[2ch] text-center">{value}</div>
        <button
          type="button"
          onClick={handleInc}
          disabled={value >= max}
      ...
      <div className="flex justify-between items-baseline mt-2 px-1">
        <span className="text-xs uppercase font-bold text-muted-foreground/70 tracking-wider">{label}</span>
        {subLabel && (
          <span className="text-xs font-medium text-primary animate-in fade-in duration-300">{subLabel}</span>
        )}
      </div>
    </div>
  );
...
export function ScheduleStep({
  ...
  const effectiveWeeklyFrequency = frequencyType === 'flexible' ? frequency.timesPerWeek || 3 : selectedDaysCount;
              ...
              {isDelayed ? (
                <div className="bg-blue-950/20 border border-blue-900/30 rounded-xl p-4 flex gap-3">
                  ...
                  <div className="text-sm text-blue-300/80 leading-relaxed">
                    ...
                    . Pacjent otrzyma powiadomienie <strong>{format(startDate, 'd MMMM', { locale: pl })}</strong>.
                  </div>
                </div>
              ) : (
                <div className="bg-surface-light/30 border border-border/30 rounded-xl p-4 flex gap-3">
                  ...
                  <div className="text-sm text-muted-foreground leading-relaxed">
                    Plan aktywny przez <strong>{durationDays} dni</strong>. Pacjent otrzyma powiadomienie w dniu startu.
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="space-y-6">
              ...
              {frequencyType === 'flexible' ? (
                ...
                <div className="bg-surface-light/50 border border-border/40 rounded-xl p-4 text-center">
                  <div className="font-bold text-3xl text-foreground tabular-nums">{selectedDaysCount}</div>
              ...
              <Collapsible open={frequencyType === 'specific'}>
                ...
                <CollapsibleContent>
                  ...
                  <p className="text-xs text-muted-foreground mt-3 text-center">
                    Wybrano: <span className="font-medium text-foreground">{selectedDaysCount}</span>{' '}
                    {pluralizeDay(selectedDaysCount)}
                  </p>
                </CollapsibleContent>
              </Collapsible>
            </div>
