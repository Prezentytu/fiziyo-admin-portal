...
export function ExerciseSearchPopover({
  ...
  const title = isRegression ? 'Wybierz łatwiejszą wersję' : 'Wybierz trudniejszą wersję';

  return (
    <Popover open={isOpen} onOpenChange={handleOpenStateChange}>
      <PopoverTrigger asChild>{children}</PopoverTrigger>
      <PopoverContent className={cn('w-[400px] p-0', className)} align="start" sideOffset={8}>
        <Command className="rounded-lg border-0">
          ...
          <CommandList className="max-h-[350px]">
            ...
            {/* Empty state */}
            <CommandEmpty>
              {searchQuery.length < 2 ? 'Wpisz min. 2 znaki aby wyszukać' : 'Nie znaleziono ćwiczeń'}
            </CommandEmpty>
          ...
          {hoveredExercise && <ExercisePreviewPanel exercise={hoveredExercise} />}
        </Command>
      </PopoverContent>
    </Popover>
  );
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {isAISuggestion && <Sparkles className="h-3 w-3 text-amber-500 shrink-0" />}
        </div>
        <div className="flex items-center gap-1.5 mt-0.5">
          {exercise.difficultyLevel && (
            <Badge
              variant="outline"
              className={cn('text-[9px] px-1.5 py-0', DIFFICULTY_COLORS[exercise.difficultyLevel as DifficultyLevel])}
            >
              {DIFFICULTY_LABELS[exercise.difficultyLevel as DifficultyLevel]}
            </Badge>
          )}
          {exercise.mainTags?.[0] && (
            <span className="text-[10px] text-muted-foreground truncate">{exercise.mainTags[0]}</span>
          )}
        </div>
        {reason && <p className="text-[10px] text-muted-foreground mt-0.5 truncate">{reason}</p>}
      </div>

      {/* Confidence */}
      {confidence !== undefined && (
        <Badge variant="outline" className="text-[9px] shrink-0 border-amber-500/40 text-amber-600">
    ...
    <div className="absolute right-0 top-0 translate-x-full w-[200px] ml-2 p-2 bg-popover border border-border rounded-lg shadow-lg">
      ...
      <div className="flex flex-wrap gap-1">
        {exercise.difficultyLevel && (
          <Badge
            variant="outline"
            className={cn('text-[9px]', DIFFICULTY_COLORS[exercise.difficultyLevel as DifficultyLevel])}
          >
            {DIFFICULTY_LABELS[exercise.difficultyLevel as DifficultyLevel]}
          </Badge>
        )}
