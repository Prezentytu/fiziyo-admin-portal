...
import { Sparkles, Loader2, X, Undo2, CheckCircle2, AlertCircle, Zap } from 'lucide-react';
 ...
 */
async function mockAIVideoAnalysis(exercise: AdminExercise): Promise<AIVideoAnalysisResponse> {
  // Użyj istniejącego serwisu AI jako fallback
  const suggestion = await aiService.getExerciseSuggestion(exercise.name, exercise.mainTags || []);
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col">
          {status === 'idle' && (
            <>
              <span className="text-sm font-medium text-foreground">Brakujące dane?</span>
          ...
          {status === 'complete' && lastResult && (
            <>
              ...
              <span className="text-xs text-muted-foreground">Fioletowe pola zostały wypełnione przez AI</span>
            </>
          )}

          {status === 'error' && (
            <>
              <span className="text-sm font-medium text-destructive">Błąd analizy</span>
              <span className="text-xs text-muted-foreground">{errorMessage || 'Spróbuj ponownie'}</span>
            </>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {status === 'complete' && (
          <Button onClick={handleUndo} variant="outline" size="sm" className="gap-2" data-testid="ai-analysis-undo-btn">
