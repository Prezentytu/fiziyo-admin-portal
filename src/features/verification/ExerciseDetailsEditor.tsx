...
export function ExerciseDetailsEditor({
  ...
  const handleNumberChange = (value: string, setter: (val: number | null) => void) => {
    if (value === '' || value === null) {
      setter(null);
    } else {
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) => handleNumberChange(e.target.value, onRestBetweenSetsChange)}
                  placeholder="60"
                />
              </div>
              <div className="space-y-1">
                <Label className="text-xs">Tempo (np. 2-1-2)</Label>
                <Input value={tempo} onChange={(e) => onTempoChange(e.target.value)} placeholder="2-1-2" />
              </div>
            </div>
