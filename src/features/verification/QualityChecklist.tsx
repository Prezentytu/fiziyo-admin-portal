...
function CheckItem({ id, label, description, icon, value, onChange, autoDetected, compact = false }: CheckItemProps) {
  ...
  return (
    <div className="flex items-start justify-between gap-4 py-3 border-b border-border/40 last:border-0">
      ...
      <div className="flex items-center gap-3">
        {value !== null && (
          <span className={cn('text-xs font-medium', value ? 'text-emerald-500' : 'text-destructive')}>
            {value ? 'Tak' : 'Nie'}
          </span>
        )}
        <Switch
...
export function QualityChecklist({
  ...
  const descriptionAutoDetect = hasDescription
    ? 'pass'
          ...
          <CollapsibleContent className="pt-2">
            ...
            {showNotes && onInternalNotesChange && (
              <div className="mt-3 space-y-1.5">
                <Label className="text-xs text-muted-foreground">Notatki wewnętrzne (co jest nie tak?)</Label>
                <Textarea
                  value={internalNotes}
      ...
      <CardContent className="pt-0">
        ...
        {showNotes && onInternalNotesChange && (
          <div className="mt-4 space-y-2">
            <Label className="text-sm text-muted-foreground">Notatki wewnętrzne (co jest nie tak?)</Label>
            <Textarea
              value={internalNotes}
              onChange={(e) => onInternalNotesChange(e.target.value)}
              placeholder="Opisz problemy z jakością..."
