...
export function ExerciseSetDetailPage({ params }: SetDetailPageProps) {
  const { id } = use(params);
  ...
  const backToListHref =
    searchParams.get('from') === 'patient-plans' ? '/exercise-sets?filter=patient-plans' : '/exercise-sets';
  ...
  const handleQuickExerciseUpdate = async (mapping: ExerciseMapping, patch: Partial<ExerciseExecutionCardData>) => {
  ...
  if (error || !exerciseSet) {
    return (
      <ErrorState
        title={error ? 'Nie udało się wczytać zestawu' : 'Nie znaleziono zestawu'}
        description={error?.message}
        onRetry={error ? () => void refetch() : () => router.push(backToListHref)}
      />
    );
  }
      ...
      {assignments.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            {!isTemplateSet && (
              <Button
                data-testid="set-detail-assign-patients-btn"
                size="sm"
                onClick={() => setIsAssignDialogOpen(true)}
              >
