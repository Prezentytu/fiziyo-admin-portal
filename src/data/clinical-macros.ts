...
export function getMacrosForTags(exerciseTags: string[]): ClinicalMacro[] {
  ...
  const tagSpecificMacros = CLINICAL_MACROS.filter(
    (macro) =>
      macro.tags.length > 0 &&
      macro.tags.some((macroTag) => normalizedTags.some((exTag) => exTag.includes(macroTag.toLowerCase())))
  );
  ...
  const uniqueMacros = allMacros.filter((macro, index, self) => index === self.findIndex((m) => m.id === macro.id));
