...
Każdy nowy spec ma frontmatter z wskaźnikami na dowody (status zostaje w indeksie):

```yaml
---
spec: SPEC-0xx
repo: fiziyo-admin-portal # albo cross-repo
issues: [] # np. fiziyo-admin-portal#7
prs: [] # dopisywane po merge
board: # opcjonalnie ID z fizjo-app/.ai/BOARD.md
---
```

- Odwołuj się do PR-ów / commitów i plików, nie wklejaj kodu (wyjątek: SDL kontraktu GraphQL).
- Załączniki jako snapshot w `.ai/specs/assets/SPEC-0xx/`; bez linków do Google Doc / Loom / czatu.
- Changelog nazywa PR: `### 2026-09-12 — #34`.
- Spec z issue (`from-przemek`, `from-sync`): numer w `issues`, cytat zgłoszenia ≤25 słów w celu biznesowym.
- Dryf spec ↔ kod sprawdza tygodniowa automation (`docs/architecture/cloud-agent-policy.md`).
...
| 019 | [Second Gen Skills Open-Mercato](SPEC-019-2026-06-12-second-gen-skills-open-mercato.md)               | Wykonywalne skille, manifest lint, runy i audyty               | implemented |
