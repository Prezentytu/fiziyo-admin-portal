...
- **Problem**: Po generacji AI foty na detalu ćwiczenia przycisk Zapisz zostawał wyszarzony; odblokowywał się dopiero po zmianie innego pola.
...
- **Reguła**: Jeśli media idą osobną mutacją + refetch, dirty tracking pól tekstowych musi przetrwać zmianę referencji `source`; UI ma powiedzieć, że media już są zapisane.
...
### 2026-06-24 - Inline edycja strony szczegółów = wzbogacenie istniejącego widoku, nie nowy panel

- **Kategoria**: `React` | `UI/UX`
- **Problem**: Pierwsza próba inline-edycji szczegółów ćwiczenia podmieniła cały widok na `VerificationEditorPanel` (płaska lista wszystkich pól + edytor JSON `enrichmentData` + sekcje "Zaawansowane"). Użytkownik odrzucił to jako zbyt skomplikowane — chciał TEN SAM widok read-only (parametry, zakładki opisów, kroki, enrichment) z możliwością edycji.
- **Przyczyna**: Reużycie ciężkiego komponentu weryfikacyjnego zmieniło prezentację i ujawniło pola/JSON, które na stronie detalu mają pozostać czytelnym, kuratorowanym widokiem.
- **Rozwiązanie**: Zachowano oryginalny layout (`ExerciseParametersPanel`, zakładki pacjent/fizjo/notatki, `ExerciseExecutionSteps`, `ExerciseAudioCues`, `EnrichmentDisplay`). Tryb edycji włącza JEDEN przycisk "Edytuj ćwiczenie" w menu (`isEditMode`), a komponenty wyświetlające dostają opcjonalny tryb edycji in-place (np. `ExerciseParametersPanel` z propsami `editable` + `onFieldChange` renderuje inputy/selecty w tych samych kafelkach). Autosave per pole on-blur przez `UPDATE_EXERCISE_MUTATION`; mapowanie display-key → zmienne mutacji w lokalnym `buildUpdateVariables`. Enrichment/JSON pozostaje read-only.
- **Reguła**: Gdy istnieje dopracowany widok read-only, dodawaj edycję przez wzbogacenie tych samych komponentów (opcjonalny `editable`/`onFieldChange`), a nie przez podmianę na generyczny panel formularza. Nie eksponuj edytora JSON ani sekcji "Zaawansowane" na widokach kuratorowanych. Aktywacja edycji = jeden jawny przełącznik, nie zawsze-on.

### 2026-07-01 - Komponent z `return null` przy pustej wartości blokuje edycję tego pola

- **Kategoria**: `React` | `UI/UX` | `Exercise`
- **Problem**: Po włączeniu inline-edycji parametrów ćwiczenia użytkownik nie mógł ustawić "poleceń głosowych" (audioCue), bo `ExerciseAudioCues` renderował `return null`, gdy `audioCue` i `coaching_cues` były puste — w trybie edycji sekcja całkowicie znikała, więc nie było gdzie wpisać wartości.
- **Przyczyna**: Komponent zaprojektowany wyłącznie pod read-only ("nie pokazuj puste sekcje") nie uwzględniał trybu edycji, w którym pusta wartość musi mieć widoczne pole do wypełnienia, a nie być chowana.
- **Rozwiązanie**: Dodano `editable`/`onAudioCueChange` do `ExerciseAudioCues`; warunek wczesnego wyjścia zmieniono na `!editable && !hasAudioCue && !hasCues` (chowaj tylko gdy naprawdę nic nie ma i nie edytujemy), a w trybie edycji renderuje się `Input` zamiast tekstu. W read-only + puste pokazuje teraz placeholder tekstowy zamiast całkowicie znikać.
- **Reguła**: Każdy komponent wyświetlający, który dostaje tryb `editable`, musi renderować pole wejściowe niezależnie od tego, czy wartość źródłowa jest pusta/zero/null — warunki `if (!value) return null` trzeba zamienić na `if (!editable && !value) return null` albo pokazać placeholder, inaczej edycja "od zera" jest niemożliwa.

### 2026-07-01 - Edycja enrichmentData na stronie detalu = reużycie widgetów z centrum weryfikacji, nie nowy edytor

- **Kategoria**: `React` | `UI/UX` | `Exercise` | `GraphQL`
- **Problem**: Po dodaniu edycji `audioCue` użytkownik doprecyzował, że chce edytować WSZYSTKIE możliwe parametry i kroki — czyli też dane z `enrichmentData` (kroki wykonania, wskazówki AI, typowe błędy, odczucia, bezpieczeństwo, notatki pacjenta/terapeuty, profile dawkowania, pytania feedbackowe, słowa kluczowe). Ryzyko: odtworzenie od zera całego `EnrichmentEditor`/`VerificationEditorPanel`, którego wcześniej użytkownik odrzucił jako zbyt złożony (JSON, "Zaawansowane").
- **Przyczyna**: `enrichmentData` nie ma mutacji w `UPDATE_EXERCISE_MUTATION` — jedynym kanałem zapisu jest generyczny `UPDATE_EXERCISE_FIELD_MUTATION` (`fieldName/value: String`, JSON.stringify dla obiektów), już używany w centrum weryfikacji przez `useEnrichmentDraft` + sekcje (`FeelSafetySection`, `TherapistNotesSection`, `DosingProfilesSection`, `ListEditor`).
- **Rozwiązanie**: Przeniesiono generyczne, bezstanowe widgety edycji enrichmentu (`ListEditor`, `useEnrichmentDraft`, `FeelSafetySection`, `TherapistNotesSection`, `DosingProfilesSection`) z `features/verification/enrichment/` do `components/shared/enrichment/` i reużyto ich wewnątrz kuratorowanych komponentów strony detalu (`ExerciseExecutionSteps`, `ExerciseAudioCues`, `EnrichmentDisplay`) — zamiast pisać nowy edytor od zera. Kroki wykonania i wskazówki AI uproszczono do jednej, ogólnej listy tekstowej (`ListEditor`) bez wariantów pełne/uproszczone/dla dziecka/techniczne i bez metadanych faz/priorytetu — zgodnie z wyraźną prośbą "ogólne, bez podziałów". `page.tsx` woła `useEnrichmentDraft({ enrichmentData, onFieldChange: handleFieldUpdate })`, a `handleFieldUpdate` rozpoznaje `field === 'enrichmentData'` i kieruje przez `UPDATE_EXERCISE_FIELD_MUTATION` (JSON.stringify), pomijając zwykłą ścieżkę `buildUpdateVariables`/`UPDATE_EXERCISE_MUTATION`.
- **Reguła**: Zanim zaczniesz pisać nowy edytor dla ustrukturyzowanych danych (JSONB/enrichment), sprawdź czy analogiczna edycja już istnieje w innym module (tu: centrum weryfikacji) — przenieś generyczną logikę do `components/shared/`, a widoki kuratorowane dostosuj wizualnie przez proste opakowanie (`editable` + `setPath`/`persist`), nie duplikuj mechanizmu zapisu. Przy prośbie "ogólne, bez podziałów" wybieraj najprostszy wspólny mianownik danych (jedna lista tekstowa) i pisz tylko do jednego kanonicznego pola, nie czyszcząc pozostałych wariantów w danych.

### 2026-07-07 - Redesign edytora ćwiczenia v3: jeden model formularza + sticky zapis zamiast autosave-on-blur

- **Kategoria**: `React` | `UI/UX` | `Exercise` | `GraphQL`
- **Problem**: Strona detalu miała rozproszony autosave-on-blur (każde pole core przez `UPDATE_EXERCISE_MUTATION`, enrichment przez `UPDATE_EXERCISE_FIELD_MUTATION`), mieszała dane v2/v3, eksponowała user-facing "enrichment/Dane rozszerzone", a czas ćwiczenia miał edytowalne `duration` obok `executionTime`. Brakowało też edytowalnych pól v3 (`patient.summary`, `safety.intensity_guide`, `ai.problems/suitable_for/contraindicated_for`) i core (`videoUrl`, obciążenie kg).
- **Przyczyna**: Organiczny rozrost widoku — kolejne pola dokładane per-komponent bez wspólnego modelu stanu ani normalizacji do jednego schematu.
- **Rozwiązanie**: Dodano hook `useExerciseEditorForm` (jeden draft core + jeden draft v3 znormalizowany raz `toV3` na wejściu, dirty tracking, skoordynowany `save`: tylko zmienione pola core → `UPDATE_EXERCISE_MUTATION`, oczyszczony `cleanupEnrichment` → `UPDATE_EXERCISE_FIELD_MUTATION`). Parametry: `ExerciseParametersEditor` z segmented Powtórzenia/Czas, jedno pole `executionTime` (rep_duration_seconds), obciążenie w kg mapowane na `loadType/Value/Unit/Text`, read-only kafelki "Czas serii"/"Czas trwania". Sekcje uporządkowane wg ważności (parametry → instrukcja pacjenta → kroki → audio → odczucia/błędy → bezpieczeństwo → dla terapeuty → metadane), bez słowa "enrichment". Sticky pasek "Zapisz zmiany" z dirty state. `safety.intensity_guide` w `toV3` stało się polem niezależnym, a nie fallbackiem `stop_if`.
- **Reguła**: Dla widoków z wieloma polami z różnych źródeł (core + JSONB) trzymaj JEDEN model formularza z dirty trackingiem i jednym jawnym zapisem, zamiast autosave-on-blur per pole. Inicjalizuj stan ze STABILNEJ referencji Apollo (`data?.exerciseById`), nie z obiektu przeliczanego co render (np. `mapExerciseTagsToObjects`), inaczej `useEffect(setState)` kasuje edycje na każdym renderze. Czas ćwiczenia: edytuj `executionTime` (czas 1 powtórzenia), a czas serii/łączny pokazuj jako read-only wyliczenia — nie dubluj z `duration`.

### 2026-07-07 - check-testid-coverage ucina tag na pierwszym `>` → arrow `=>` przed data-testid = fałszywy alarm

- **Kategoria**: `Tooling` | `Testing`
- **Problem**: Po przebudowie `page.tsx` guard `check:testids` zgłaszał jako "brak data-testid" elementy, które JAWNIE miały `data-testid` (m.in. w `ExerciseDetailSections`, `FeelSafetySection`).
- **Przyczyna**: `readOpeningTag` skleja linie tagu tylko do pierwszej zawierającej `>`. Prop `onChange={(event) => ...}` / `onClick={() => ...}` zawiera `=>` (czyli `>`), więc chunk kończy się PRZED linią z `data-testid` umieszczonym niżej. Stąd setki wpisów w allowlist mimo obecnego testid.
- **Rozwiązanie**: Przy przesunięciach linii regeneruj bazę: `node scripts/check-testid-coverage.mjs --update-allowlist` (allowlist jest wersjonowana i to sankcjonowany mechanizm). Realnie brakujący testid (np. przycisk błędu) dodaj ręcznie. Dla nowego kodu, gdy chcesz uniknąć wpisu w allowlist, umieść `data-testid` jako PIERWSZY atrybut (przed `onChange/onClick` z `=>`).
- **Reguła**: Guard `data-testid` to heurystyka tekstowa wrażliwa na `=>` przed atrybutem i na numery linii — po zmianach UI zawsze uruchom `check:testids`, a przy przesunięciach linii zregeneruj allowlistę zamiast walczyć z fałszywymi trafieniami.

### 2026-07-07 - Reużycie edytora ćwiczenia w 3 widokach weryfikacji zamiast osobnego panelu per widok

- **Kategoria**: `React` | `UI/UX` | `Exercise` | `GraphQL`
- **Problem**: Trzy widoki weryfikacji (`/verification/[id]`, `/organization/verification/[id]`, `/verification/organizations/[id]`) każdy renderowały ten sam `VerificationEditorPanel` z lokalnym `handleFieldUpdate`/`onFieldChange` per pole (autosave-on-blur), duplikując mapowanie pól i mając rozjazd wartości enum (`side` uppercase vs lowercase, `difficultyLevel` z nieistniejącym w backendzie `BEGINNER`, `rangeOfMotion` jako zamknięty dropdown zamiast wolnego tekstu z `DOMAIN_MODEL.md`).
