...
export function checkAdapter(root) {
  ...
  if (
    !Array.isArray(adapter.externalRepositories) ||
    adapter.externalRepositories.some((repo) => !/^[\w.-]+\/[\w.-]+$/.test(repo))
  )
    errors.push('Invalid external repositories');
  ...
  for (const field of ['instructions', 'surfaces']) {
    if (!Array.isArray(adapter[field]) || !adapter[field].length) errors.push(`Missing ${field}`);
      ...
      for (const relative of adapter[field])
        if (!local(relative)) errors.push(`Missing or unsafe ${field}: ${relative}`);
  }
  ...
  if (!Array.isArray(adapter.checks) || !adapter.checks.length) errors.push('Missing checks');
    ...
    for (const check of adapter.checks) {
      if (!local(check.cwd) || !Array.isArray(check.args) || check.args.some((arg) => typeof arg !== 'string')) {
        ...
        if (check.cwd !== '.' || !Object.hasOwn(pkg.scripts ?? {}, check.args[1]))
          errors.push(`Missing npm script: ${check.args[1]}`);
      } else if (check.command === 'node' && check.args[0] === '--test' && check.args.length > 1) {
        for (const script of check.args.slice(1))
          if (!local(path.join(check.cwd, script))) errors.push(`Missing test: ${script}`);
  ...
  const validateSurfaceList = (field, { shouldExist }) => {
    const list = adapter[field];
    ...
    for (const relative of list) {
      if (!isSafeRelative(relative)) {
        errors.push(`Unsafe ${field}: ${relative}`);
        continue;
      }
      if (seen.includes(relative)) errors.push(`Duplicate ${field}: ${relative}`);
      seen.push(relative);
      const present = local(relative);
      if (shouldExist && !present) errors.push(`Missing ${field}: ${relative}`);
      if (!shouldExist && present) errors.push(`${field} must not exist: ${relative}`);
    }
    return seen;
  };

  const overrides = validateSurfaceList('portableSurfaceOverrides', { shouldExist: true });
  const excluded = validateSurfaceList('portableSurfaceExcluded', { shouldExist: false });
  for (const relative of excluded) {
    if (overrides.includes(relative)) {
      errors.push(`Path listed in both portableSurfaceOverrides and portableSurfaceExcluded: ${relative}`);
    }
  }
