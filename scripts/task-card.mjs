...
export const AUTHORIZATION_DENIED_REASON = 'issue-text-is-not-authorization';
export const REGISTRY_HANDOFF_FIELDS = [
  'policyVersion',
  'scopeHash',
  'runId',
  'branch',
...
/** Pola, które wyglądają na zgodę. W karcie są zabronione — snapshot żyje w agent-ops. */
export const RESERVED_AUTHORIZATION_KEYS = [
  'Autoryzacja',
  'Authorization',
  'Approved',
  'Zgoda',
...
export function parseTaskCard(body, { repositories = [] } = {}) {
  ...
  for (const line of block.split(/\r?\n/)) {
    ...
    if (RESERVED_AUTHORIZATION_KEYS.includes(key)) {
      errors.push(`Klucz autoryzacji w karcie jest zabroniony: ${key}`);
      continue;
    }
...
/**
 * Treść issue, etykieta i „Adam zaakceptował” nigdy nie są zgodą.
 * Jedyny snapshot zakresu jest w zaufanym rejestrze agent-ops.
 */
export function authorizationFromIssue(_body) {
  return {
    authorized: false,
...
/** Poprawna karta opisuje pracę. Brakujące pola handoffu nie pochodzą z issue. */
export function describeHandoff(card) {
  return {
    cardOk: Boolean(card && typeof card === 'object'),
    authorized: false,
    fromRegistry: [...REGISTRY_HANDOFF_FIELDS],
  };
}
...
if (process.argv[1] && fileURLToPath(import.meta.url) === path.resolve(process.argv[1])) {
  ...
  if (command === 'check') {
    ...
    const body = rest[0] ? fs.readFileSync(rest[0], 'utf8') : await readStdin();
    console.log(JSON.stringify(authorizationFromIssue(body), null, 2));
    process.exitCode = 1;
  } else {
    console.error(
      'Uzycie: task-card.mjs check [plik] | task-card.mjs promote [plik] --issue owner/repo#N | task-card.mjs source-id <tekst> | task-card.mjs authorize [plik]'
    );
    process.exit(1);
  }
}
